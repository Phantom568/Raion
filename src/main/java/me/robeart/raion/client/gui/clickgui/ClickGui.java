...
import me.robeart.raion.client.gui.clickgui.theme.JakeTheme;
...
public final class ClickGui extends GuiScreen {
	...
	public Theme getThemeMode() {
		switch (ClickGuiModule.INSTANCE.getMode().getValue()) {
			case "Jake":
				return new JakeTheme();
			case "Raion":
				return new RaionTheme();
		}
		return this.theme;
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (!this.getTheme().equals(getThemeMode())) {
			this.theme = getThemeMode();
			this.mainPanel = new MainPanel("Client", 50, 50, this.theme);
			this.theme.mainConstructor(this, this.mainPanel);
